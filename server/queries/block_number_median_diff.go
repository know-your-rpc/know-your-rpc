...
type BlockNumberQueryTemplate struct {
	Interval int `validate:"required,number,lt=72,gt=0"`
...
func CreateBlockNumberDiffFromMedianQuery(serverContext *server.ServerContext) func(w http.ResponseWriter, r *http.Request) {
	queryTemplate, err := template.New("query").Parse(`SELECT date_bin(INTERVAL '{{.BinTime}} seconds', time) as _time, min("diffWithMedian") as minmedian, max("diffWithMedian") as maxmedian , "rpcUrl"
				FROM "blockNumber"
				WHERE
				time >= now() - interval '{{.Interval}} hour'
				AND
				"isError" = 'false'
				AND
				"chainId" = '1'
	...
	return func(w http.ResponseWriter, r *http.Request) {
		...
		interval, err := strconv.Atoi(GetQueryParam(queryParams, "interval", DEFAULT_INTERVAL))

		if err != nil {
			fmt.Printf("failed to read interval value error=%s\n", err.Error())
			w.WriteHeader(http.StatusBadRequest)
			return
		}

		intervalInSecs := float64(interval * 3600)

		binTime := int(1.0 / (MAX_POINTS / intervalInSecs * POINTS_PER_SECOND))
		fmt.Printf("binTime=%d \n", binTime)

		queryTemplateInput := BlockNumberQueryTemplate{
			Interval: interval,
			BinTime:  binTime,
		}
